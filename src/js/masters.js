...
function renderKPI() {
  ...
  const thisMonthGoals = state.progress.filter(
    (p) => p.year_month_goal === currentMonth,
  ).length;
  ...
  document.getElementById("kpi-masters-count").textContent =
...
function renderMasterList() {
  ...
  container.querySelectorAll(".stage-item").forEach((el) => {
    el.querySelector(".stage-checkbox")?.addEventListener(
      "click",
      async (e) => {
        e.stopPropagation();
