...
function renderCalendar() {
  ...
  cells.forEach((d) => {
    ...
    events.slice(0, 3).forEach((ev) => {
      ...
      chip.textContent =
        ev.type === "exam" ? ev.label : `第${getUnitNumber(ev.unitId)}コマ`;
      chip.title = ev.label;
      if (ev.type === "unit") {
        chip.addEventListener("click", (e) => {
          e.stopPropagation();
          openDayModal(dStr, events);
        });
      }
  ...
  const allEvents = [
    ...scheduledUnits.map((u) => {
      ...
      return {
        ...
        type: "unit",
        label: `${course?.name || "東進"} 第${u.unit_number}コマ`,
        done: u.is_completed,
        unitId: u.id,
      };
    }),
