...
function renderCourseDetail() {
  ...
  const scheduled = units.filter(
    (u) => u.scheduled_date && !u.is_completed,
  ).length;
...
function openUnitModal(unitId) {
  ...
  document.getElementById("unit-modal-title").textContent =
    `第${unit.unit_number}コマ — ${course.name}`;
  document.getElementById("unit-completed-check").checked = unit.is_completed;
  document.getElementById("unit-completed-date").value =
    unit.completed_at || "";
  document.getElementById("unit-scheduled-date").value =
    unit.scheduled_date || "";
  document.getElementById("unit-completed-date").disabled = !unit.is_completed;

  // completed_at visibility
  document.getElementById("unit-completed-date-row").style.display =
    unit.is_completed ? "" : "none";
...
function renderMonthGoalPanel() {
  ...
  const goal = state.goals.find(
    (g) => g.course_id === selectedCourseId && g.year_month === currentMonth,
  );
  const units = state.units.filter((u) => u.course_id === selectedCourseId);
  const doneThisMonth = units.filter(
    (u) => u.is_completed && u.completed_at?.startsWith(currentMonth),
  ).length;
  const goalUnits = goal?.goal_units ?? 0;
  const pct = goalUnits
    ? Math.min(100, Math.round((doneThisMonth / goalUnits) * 100))
    : 0;

  document.getElementById("goal-month-label").textContent =
