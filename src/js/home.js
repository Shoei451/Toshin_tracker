...
function renderKPI() {
  ...
  const scheduledThisMonth = state.units.filter((u) =>
    u.scheduled_date?.startsWith(ym),
  ).length;
  ...
  const scheduledNextMonth = state.units.filter((u) =>
    u.scheduled_date?.startsWith(nextMonth),
  ).length;
  ...
  if (bar) {
    bar.style.width = pct + "%";
    bar.className =
      "progress-fill" + (pct === 100 ? " done" : pct < 30 ? " warn" : "");
  }
...
function renderCourseProgress() {
  ...
  const goal = (courseId) => {
    const g = state.goals.find(
      (g) => g.course_id === courseId && g.year_month === ym,
    );
  ...
  container.innerHTML = state.courses
    .map((course) => {
      ...
      const doneThisMonth = units.filter(
        (u) => u.is_completed && u.completed_at?.startsWith(ym),
      ).length;
      const goalPct = g
        ? Math.min(100, Math.round((doneThisMonth / g) * 100))
        : null;
...
function renderMonthlyGoals() {
  ...
  const rows = state.courses
    .map((course) => {
      const g = state.goals.find(
        (x) => x.course_id === course.id && x.year_month === ym,
      );
  ...
  container.innerHTML = rows
    .map(({ course, goalUnits, doneThisMonth }) => {
